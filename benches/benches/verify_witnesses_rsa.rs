    ...
    <P as RsaKVACParams>::RsaGroupParams: Sync,
{
    ...
    // Create key-values
    let k = hash_to_integer::<H>(&[BLS381Fr::zero()], P::KEY_LEN);
    let v = hash_to_integer::<H>(&[BLS381Fr::zero()], P::VALUE_LEN);
    ...
    for log_len in range_lengths.iter() {
        ...
        for num_cores in cores.iter() {
            ...
            update_pool.install(|| {
                ...
                let mut kvac = RsaKVAC::<P, H, H, BigNatTestParams>::new();
                for _ in 0..len {
                    kvac._update_without_proof(k.clone(), v.clone()).unwrap();
                }
                let (_, witness) = kvac.lookup(&k).unwrap();
                ...
                csv_writer.flush().unwrap();
                let start = Instant::now();
                assert!(RsaKVAC::<P, H, H, BigNatTestParams>::verify_witness(&k, &Option::Some(v.clone()), &kvac.commitment, &witness).unwrap());
                let end = start.elapsed().as_millis();
                csv_writer
                    .write_record(&[
                        scheme_name.clone(),
...
fn main() {
    ...
    {
        println!("Usage: ``cargo bench --bench verify_witnesses_rsa --  [--ranges <RANGE_LEN>...][--num_cores <NUM_CORES>...]``");
        return;
    } else {
